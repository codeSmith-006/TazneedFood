...
import { SearchOutlined, EyeOutlined, PhoneOutlined, EnvironmentOutlined, UserOutlined, ShoppingOutlined } from '@ant-design/icons';
          ...
          <table className="w-full">
            <thead className="bg-secondary/50">
              <tr>
                ...
                <th className="text-left px-4 lg:px-6 py-4 text-sm font-medium text-muted-foreground hidden md:table-cell">Payment</th>
                <th className="text-left px-4 lg:px-6 py-4 text-sm font-medium text-muted-foreground hidden lg:table-cell">Date</th>
                <th className="text-left px-4 lg:px-6 py-4 text-sm font-medium text-muted-foreground">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              <AnimatePresence>
                {filteredOrders.map((order, index) => (<motion.tr key={order.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ delay: index * 0.05 }} className="hover:bg-secondary/30 transition-colors">
          ...
          {selectedOrder && (<div className="space-y-6 mt-4">
              ...
              <div>
                ...
                <div className="space-y-3">
                  {selectedOrder.items.map((item, index) => (<div key={index} className="flex items-center gap-4 p-3 bg-secondary/30 rounded-lg">
              ...
              <div className="border-t border-border pt-4 space-y-2">
                ...
                {selectedOrder.discount > 0 && (<div className="flex justify-between text-sm text-green-600">
              ...
              {/* Notes */}
              {selectedOrder.note && (<div className="bg-yellow-50 dark:bg-yellow-900/20 rounded-lg p-4">
                  <p className="text-sm font-medium text-yellow-800 dark:text-yellow-200">Order Note:</p>
                  <p className="text-sm text-yellow-700 dark:text-yellow-300">{selectedOrder.note}</p>
                </div>)}
