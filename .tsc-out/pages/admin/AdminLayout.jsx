...
import { DashboardOutlined, ShoppingOutlined, OrderedListOutlined, AppstoreOutlined, LogoutOutlined, MenuOutlined, CloseOutlined } from '@ant-design/icons';
const AdminLayout = () => {
    ...
    const getPageTitle = () => {
        const currentNav = navItems.find(item => item.exact ? location.pathname === item.path : location.pathname.startsWith(item.path) && item.path !== '/admin');
    ...
    return (<div className="min-h-screen bg-background flex">
      ...
      {/* Sidebar */}
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 bg-card shadow-lg transition-all duration-300 
          ...
          <nav className="flex-1 py-4">
            <ul className="space-y-1 px-2">
              {navItems.map((item) => (<li key={item.path}>
                  <Link to={item.path} onClick={() => setMobileMenuOpen(false)} className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${isActive(item.path, item.exact) || (item.exact && location.pathname === item.path)
